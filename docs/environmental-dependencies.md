...
|---|---|---|---|---|
| **WebGL2** | MapLibre (`MapView.jsx`) | Map cannot render at all | try/catch on construction → `onUnavailable`; `FeatureBoundary` for later throws; `map.on("error")` for async death. Map zone leaves the layout, feed goes full width, one line says so | `mapContainment.test.jsx` (8 tests, both failure shapes) + **`npm run verify:agent-browser`** (real browser, real bundle, WebGL stubbed off) |

The row above says WebGL**2** as of 2026-09-14: MapLibre 6 dropped WebGL 1 entirely, so a browser that
offers only WebGL 1 now lands in the same place as a browser with no WebGL at all — the map zone leaves
the layout and the feed goes full width. That path was already built and tested for the 2026-08-13 bug,
so the upgrade widened who reaches it rather than adding a new way to fail. WebGL 1-only browsers are
rare (pre-2021 Safari and old Android WebViews), but the Facebook in-app browser row below is the one to
watch, since that is where the oldest engines arrive.
...
| **The Facebook in-app browser** | every arrival from a group post — Facebook opens links inside its own browser, not Safari or Chrome | Unknown until tested, which was the problem: no reviewer's browser can produce it, the same shape as the WebGL bug above. ~100% of a parents-group post's arrivals land here | Nothing specific to it; it inherits every containment in this table | **`npm run verify:fb-webview`** (2026-09-08) — the built site under both engines Facebook embeds (WebKit for iOS, Chromium for Android), each with the in-app browser's user agent, a phone viewport, touch input and the `fbclid` Facebook staples onto every link. Checks the feed, the map, storage, tap filtering, shared `/e/` links, and that the `?src=` tag reaches both signup asks. **`npm run verify:fb-webview`** (2026-09-08) for the engine, plus **a real iPhone, 2026-09-10** for the app around it: Batu opened `?src=verify&lens=family_kids` from a Facebook post on his own phone — the map drew, the Family & Kids chip came up already active, and a card opened. The Android app is still unchecked; its engine is not — see Known gaps |
| **A storage jar shared with the reader's real browser** (in-app browsers) | `returnVisit.js` (R0 retention), `firstVisitOrientation.js`, and every per-channel return comparison | Facebook's and Instagram's in-app browsers keep their **own** storage, separate from Safari's or Chrome's. Nothing throws and every feature works — but a reader who taps a group post and later returns in their real browser is **two different people** to the retention sensor. It deflates return rates on webview-borne channels (Q2's parents posts, where ~100% of taps arrive this way) relative to email-borne org channels, so a naive cross-channel comparison reads a measurement artifact as a channel result | **Measured, not fixed (2026-08-19).** `webviewContext.js` tags every event with `webview: facebook \| instagram \| other` via the same single `setEventContext` call that carries `src`, so a Q2-vs-org comparison can be split instead of caveated. Nothing client-side can merge the jars; the same ruling shape as the dropped cookie mirror above — measure the bias, label the read | `webviewContext.test.mjs` (7 tests, verbatim real UA strings) + browser-verified end to end 2026-08-19: a spoofed Facebook UA produced `{src: parents-wgbk, webview: facebook}` on a `return_visit` payload. **Still open, and it needs a phone:** whether the product itself renders correctly inside the FB app on real iOS and Android — the hostile-context class that produced the 8/13 map bug. Batu's ~15-minute check, Q2 precondition |
...
- **No real-device matrix.** Everything is verified via stubs, jsdom, one browser pane, or
  ...
  - The **Facebook in-app browser** is covered on iOS and by engine everywhere.
    `npm run verify:fb-webview` (2026-09-08) drives the built site with the two engines Facebook
    actually embeds, so an engine-level break — the class the 2026-08-13 WebGL bug belonged to —
    would fail a run. On 2026-09-10 Batu opened a tagged link from a Facebook post on his own
    iPhone and the app around the engine held up too: the map drew, the lens deep link applied,
    and a card opened. **What is left is the Android app specifically** — the same wrapper
    questions (a link that asks for a new tab, whether its data store survives between sessions,
    how much room its toolbars leave) on Android's version of it. That is a smaller gap than the
    row started with, because Android's engine is Chromium and the automated check already drives
    it, but it is not zero: the wrapper is separate software from the engine it embeds.
