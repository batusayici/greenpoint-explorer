...
| 4D-1: Geometry validation and gap audit | Proposed next authorization only; not executable yet. | Would make existing 4B/4C corridor geometry inspectable and confidence-labeled before POIs, facade evidence, or storefront anchors. Expected outputs: generated geometry validation/gap report, QA inspector or overlay improvements, rendered object ID, source footprint/building ID, corridor side, relative order/block position, source-backed/inferred/manual status, height/massing confidence, gap/block-break status, address/building ambiguity if known, and POI/facade-anchor eligibility states. | No Foursquare, local-directory calls, POI overlay, facade imagery ingestion, storefront anchors, asset registry, visual-system work, new dependencies without approval, source expansion, runtime/UI/verifier/data/package/source changes unless Batu explicitly opens those boundaries. | If authorized later: relevant report/verifier checks, existing source fixture/compiler checks if referenced, app/browser smoke only if runtime changes are opened, and `git diff --check`. | Proposed fixture/report/runtime-inspection boundary only after Batu approval. | Batu must explicitly authorize 4D-1 before implementation. Acceptance: Batu can inspect a building and understand what it is, where it came from, geometry confidence, allowed/blocked claims, and future POI/facade anchor safety. |
| 4D-2: Claim ladder / matching contract | Proposed later phase. | Would define the claim ladder and matching contract before candidate POIs are attached: geometry container confidence, address/building ambiguity, candidate business enrichment, storefront/facade evidence, and promotion gates. | No live APIs, POI overlay, facade imagery ingestion, storefront anchors, production claims, or asset-system work. | Docs/schema sanity only if authorized later. | Contract-doc boundary. | Stop before turning candidate POI data into authoritative storefront assignment. |
| 4D-3: Candidate POI overlay | Proposed later phase. | Would attach Foursquare/local-directory/other approved POI sources as candidate enrichment only, with confidence and blocked storefront-assignment claims visible. | No authoritative storefront assignment, active-status finality, facade/frontage/entrance inference, Google/Street View asset pipeline, or production cards. | Source readiness, terms/cache/display approval checks, deterministic fixture checks, and QA smoke only if authorized later. | Candidate source/overlay boundary. | Stop before any business-to-storefront promotion. |
...
Current work packet: post-4C docs-only planning state.
...
Post-4C direction: the generic QA-only fictional facade tuning lane is closed. The proposed next authorization is `Batch 4D-1: Geometry validation and gap audit`.

Hard review gate: stop until Batu explicitly authorizes 4D-1 or a different next batch. No later Phase 4 batch is open or queued.
