...
| Greenpoint G NW cue | `context-only` / `blocked` for exact placement | May use a light symbolic/contextual subway presence only if it does not imply direct frontage or exact geometry. | Exact NW cue placement, direct `in front of Grillpoint` claim, reliance on the formerly HEIF-wrapped `northwest-subwayA.jpg` before the later extension cleanup. |
...
- Approved input reference paths from `docs/mvp-reference-images/`, including which NW files were formerly HEIF-wrapped and excluded unless re-exported.
