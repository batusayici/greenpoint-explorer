...
- Phase 4 execution gate: agents may execute only the current batch named in `docs/CURRENT_EXECUTION_BRIEF.md` or the next batch already named in that brief's pre-authorized queue, must use `docs/phase-4-execution-roadmap.md` as the operating plan, and must stop at hard Batu review gates instead of inventing, skipping, or opening unqueued batches. Current gate is open 4M-R10 with no pre-authorized queue; R11/R12 remain future candidates only.
...
87. 4M-R10: Franklin hero asset ingestion spike. Open after Batu approval; one QA-only GLB insert, structured record anchoring, procedural fallback, and Batu visual review stop.
88. 4M-R11: R9 vs R10 visual/technical comparison. Future candidate only; compare primitive modules, GLB inserts, and mixed hero-kit strategy.
89. 4M-R12: Hero-kit standardization decision. Future candidate only; codify accepted hero-kit boundaries before Manhattan, landmarks, or corridor expansion.
90. Mapillary/KartaView source-use gate. Recommended as a proposal only; not open.
91. 4L external/scaled evidence-backed QA corridor render. Deferred until Batu approves source/evidence boundaries and opens a later packet.
92. 4M production asset registry / visual system / art-direction translation. Deferred until Batu opens production visual-system scope; Qwen/Oxen may be evaluated here only as a source-safe visual-system accelerator.
93. 4N: Normal-mode promotion + recognition QA. Deferred until QA render, spatial scaffold acceptance, provenance, optional asset/art translation, and Batu promotion gates clear.
...
- Blender/GLB assets remain optional enhancement/override layers after the truth-first scaffold. The open R10 path narrows that idea to one QA-only Franklin hero insert first: the procedural scaffold remains the base; QA-only hero inserts are exceptions for high-recognition architectural features.
...
- R10 visual review decision: whether the controlled one-asset Franklin hero ingestion spike improves recognizable stylized fidelity enough to justify R11 comparison. Manhattan remains closed except narrow regression capture if shared renderer code changes.
