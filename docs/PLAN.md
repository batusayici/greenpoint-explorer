...
- **Track V follow-ups (post-validation):** reconcile the demand-test card schema with `PlaceStory`/`Landmark` into one canonical content model; fold change/civic layers + the recognizable-container fusion in as v2; refresh seed from the ~Aug 5 SSG issue; **event/subscription ingestion + business submission pipeline** (v1 is hand-curated — sustaining the "what's happening today" layer needs a repeatable intake path). (Spec: `2026-07-02-spatial-demand-test-design.md`.)
