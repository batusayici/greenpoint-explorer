...
### Phase 8 — Spine Expansion + Story Attachment — NOW (8.0 / 8.1 / 8.2 done; 8.3 / 8.4 live)

Absorbs Track-A 4.3 (corridor infill) and 5.1 (typological infill kit), and Track-B B1.

- **8.0 Structural Depth Pass — DONE + look re-gate APPROVED (2026-06-21).** Real extruded depth (3D stoops + front fire escapes), parametric and family/storey-gated, built + tested + verified on the 4-BIN pilot (pure modules `facadeDepthGates.js`/`stoopGeometry.js`/`fireEscapeGeometry.js` + `decorateInkedWall` wiring; commits `905315f..` through the craft pass). Batu approved the geometry at the Task 6 gate; design + plan in `docs/superpowers/`, decisions in `DECISION_LOG.md` (2026-06-21). **Both craft follow-ups landed + the look re-gated (Batu approved 2026-06-21):**
  - **(a) Ground-texture regen — DONE (`291d0bf`).** Batu regenerated brick + brownstone ground cells in GPT-5.5 from the regen packet with no painted stairs/door; intake fixed the baked sepia chroma (saturation pass to ~22.5 meanChroma) so they pass the tintable-neutral gate. Verified in-engine on both pilots (brick `3064677`, brownstone `3064541`): plain masonry ground band + parlor window, 3D stoop projects in front, no double-stairs.
  - **(b) Open ironwork — DONE (`3cd298d`, `1107491`).** Procedural OPEN-IRONWORK railing texture (dark balusters + top/bottom rails on transparent, tiled, tinted in-engine) — the wall shows through the balusters so it reads as iron; decks are thin platforms, ladder a slender stringer. The openwork lives in the texture, so the relief-vs-lattice variant decision is moot (resolved). Brooklyn-style 3D stair flights added.
  - **Craft pass that rode along:** stoop reads as a stoop (raked walls + newels + stone texture, `27ff7c0`); doors/window-depth consistency + party-wall clip + molded cornice (`ef02445`, `b6544fd`); true carved window/door recesses with sill + lintel cap (`b0e7b54`, `4c487d6`); modern-flat street-face/lot-line walls (`78efa7e`).
  - **Conformance cleanup — DONE (this session).** The Brooklyn fire-escape commit had introduced raw `IRON`/`IRON_DECK` hex literals that failed the 6.2.3 gate; both moved into `II_PALETTE` (`fireEscapeIron`/`fireEscapeIronDeck`). 147 tests + full `npm run verify` GREEN.
  - Basement/areaway = **Phase 8.5**, its own ref-gated mini-design pending Batu photos (photos now in `docs/reference/asset-reference/basement/`).
- **8.1 Expand along the story-dense spine — DONE (2026-06-21).** Extended the procedural scene up the curated Franklin corridor. Done in two steps:
  - **8.1a — Full kit flip (the traveling-learnings step).** Emptied `KIT_PILOT_BINS` so the 8.0 inked craft (recesses, sills, stoops, fire escapes, cornices, per-family material) applies to every kit-eligible building via the heuristic family resolver, not just the 4 pilots. Validated at scale on the ~92 existing buildings (franklin-milton + greenpoint-east): 86 brick → kit, 6 painted-masonry/warehouse → cheap (left as-is); material/recess/street-face all resolved from PLUTO + family/storey gates with **zero per-building tuning**. Verified across all four angles (inked fronts, plain typological backs). Commit `f96a2d0`.
  - **8.1b — Franklin-north corridor (Greenpoint Ave → Huron).** New `franklin-north` block via the proven recipe (descriptor + two scripted pulls + ~4 registration lines, 1 src file): 160 buildings (99% storey-backed), 140 → inked kit (brick), 20 → cheap (painted-masonry/warehouse), 26 storefronts. The kit **traveled automatically** — no per-building work. The 3 new heroes (Brouwerij Lane / Eberhard Faber building / Astral) fall in this corridor and render typological for now; hero promotion is a later, photo-gated cycle (their `building-tiers` hero flag is intact). Commits `fd68d34`, `7348849`. Full scorecard + findings in `docs/SCALING_LOG.md` (Block C). **Open curation dial:** 160 is the corridor at ~2-lot depth (matching the existing blocks); trimming to frontage-only is a Batu spine-vs-coverage call, not a recipe change.
  - **Scope decision (Batu, 2026-06-21):** kit applied to `brick`/`brownstone`/`clapboard`; `brownstone`/`clapboard` only via evidence-bound overrides; `painted-masonry`/`warehouse`/`modern-flat` left as-is. Spec/plan in `docs/superpowers/`.
  - **Evidence-backed material showcase (Batu-confirmed, 2026-06-21).** **95 Kent St (3064605) → clapboard** override restores a visible clapboard building after the flip reverted the unverified pilots to brick. **168 Franklin (3064541) is full brick** (Batu corrected an earlier dual-material reading) — reverted to the brick heuristic; no brownstone showcase building is confirmed yet.
  - **Dual-material kit capability (dormant).** A `groundFamily`/`groundTint` override (ground floor — band/stoop/door — renders in a distinct material from the wall above) was built + unit-tested, shipping the "dual-material" variation from the facade-variation-layer plan. Currently **unused** (defaults to no-op; single-material buildings byte-identical) and **never visually verified in-engine** — available for when a real dual-material building is confirmed with evidence.
  - **Storefront-stoop fix (2026-06-21).** Commercial ground floors carry a storefront (drawn by the storefront-sign system) and no longer get a residential 3D stoop — kit params carried `storefront:null` so commercial kit buildings wrongly fell into the stoop branch. Gated `drewStoop` on a `commercialGround` flag (from `classifyBuilding` ground-floor use). Verified in-engine (corridor storefronts show clean bases).
  - **Source-backed cross-street ground (2026-06-22, pushed to `origin/feat/inked-facade-look`).** The ground layer now builds real cross-streets (Kent/Java/Milton) from LION centerlines (inkn-q76z), positioned at their true intersection with Franklin, with multi-gap roadbed/curb/sidewalk clipping for N crossing streets and a crosswalk band at every real crossing. `buildCrossStreets` + `axisSegments` in `groundLayer.js`; real LION centerlines promoted into the geometry-source packet. **Limitation that 8.1c resolves:** cross-streets are still clamped to the legacy **130m context radius** (shared with the building cull), so streets/buildings beyond the circle (the whole Franklin-north corridor) stay unpaved.

- **8.1c Street-network-driven ground extent — DESIGNED 2026-06-22 (plan pending).** Resolves "buildings standing on bare ground": block extracts bypass the building cull (`sceneFrame.js:132`), so the 8.1b Franklin-north block placed 160 buildings out to ~620m, far past the 130m asphalt circle. Replaces the fixed radius with a **per-street real-centerline extent model** — each street paved along its real LION centerline for its full loaded extent, no circle, no fixed run-length; the regional data pull defines the scene boundary, so paving degrades into the neighborhood's true shape (water/parks) as it scales (H5). Locked scope: close the R10E Franklin-centerline gap opportunistically; **ground-only decouple** (building cull left untouched); missing corridor streets (Huron/Freeman/India + Franklin) via a **real LION pull**, not derivation; scoped to the loaded three blocks, not neighborhood-wide. Spec: `docs/superpowers/specs/2026-06-22-street-network-ground-extent-design.md`; decision in `DECISION_LOG.md` (2026-06-22).
...
- Franklin Ave centerline missing from the source packet (R10E finding); current Franklin roadbed is `derived: true` (real width, derived extent). **Phase 8.1c will attempt to close this** via the corridor LION pull — if it returns Franklin's centerline, the gap closes; otherwise Franklin's extent is derived from its cross-street crossings.
