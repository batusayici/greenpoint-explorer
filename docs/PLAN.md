...
- Phase 4 execution gate: agents may execute only the current batch named in `docs/CURRENT_EXECUTION_BRIEF.md` or the next batch already named in that brief's pre-authorized queue, must use `docs/phase-4-execution-roadmap.md` as the operating plan, and must stop at hard Batu review gates instead of inventing, skipping, or opening unqueued batches. Current gate is the completed 4L-Local Batu review stop with no executable batch and no queue.
...
75. 4L-Local-4: Visual review gate report. Complete and verified; stop at Batu review gate.
76. Mapillary/KartaView source-use gate. Recommended as a proposal only; not open.
77. 4L external/scaled evidence-backed QA corridor render. Deferred until Batu approves source/evidence boundaries and opens a later packet.
78. 4M: Asset registry / visual system / art-direction translation. Deferred until spatial recognizability is proven and evidence/override models are defined; Qwen/Oxen may be evaluated here only as a source-safe visual-system accelerator.
79. 4N: Normal-mode promotion + recognition QA. Deferred until QA render, spatial scaffold acceptance, provenance, optional asset/art translation, and Batu promotion gates clear.
