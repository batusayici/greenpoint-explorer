...
- Phase 4 execution gate: agents may execute only the current batch named in `docs/CURRENT_EXECUTION_BRIEF.md` or the next batch already named in that brief's pre-authorized queue, must use `docs/phase-4-execution-roadmap.md` as the operating plan, and must stop at hard Batu review gates instead of inventing, skipping, or opening unqueued batches. Current gate is the completed R10C/R10D Franklin map-truth visual review gate with no pre-authorized queue; GLB tuning, R11/R12, and Manhattan work remain future candidates only.
...
90. 4M-R10C/R10D: Franklin map-truth legibility. Complete for Batu visual review; preserves the R10B footprint mapping, suppresses corridor clutter, renders Franklin Ave as a visible cross-street slab, and captures top-down/oblique evidence for the NW/SW/SE three-corner relationship.
91. 4M-R11: R9 vs R10 visual/technical comparison. Future candidate only; compare primitive modules, GLB inserts, and mixed hero-kit strategy.
92. 4M-R12: Hero-kit standardization decision. Future candidate only; codify accepted hero-kit boundaries before Manhattan, landmarks, or corridor expansion.
93. Mapillary/KartaView source-use gate. Recommended as a proposal only; not open.
94. 4L external/scaled evidence-backed QA corridor render. Deferred until Batu approves source/evidence boundaries and opens a later packet.
95. 4M production asset registry / visual system / art-direction translation. Deferred until Batu opens production visual-system scope; Qwen/Oxen may be evaluated here only as a source-safe visual-system accelerator.
96. 4N: Normal-mode promotion + recognition QA. Deferred until QA render, spatial scaffold acceptance, provenance, optional asset/art translation, and Batu promotion gates clear.
...
- R10C/R10D map-truth visual review decision: whether the simplified Franklin / Greenpoint Ave three-corner relationship is readable and acceptable before any further GLB placement tuning, hero-kit tuning, R11 comparison, R12 standardization, Manhattan work, production mode, facade polish, materials, or asset fidelity. Manhattan remains closed except narrow regression capture if shared renderer code changes.
