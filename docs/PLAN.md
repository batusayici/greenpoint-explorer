...
- Current next task: Batu review of Phase 2DTR-4 output; Phase 2DTR-5 is proposed only if Batu accepts or revises the exact geometry source map and target scene spec.
...
5. Phase 2DTR-5 - Exact Review Geometry Fixture To Raster Prompt Adapter: proposed next step only after Batu accepts or revises the 2DTR-4 output; move the source map into a structured review-only scene fixture and deterministic raster prompt/provenance adapter.
...
- Batu review of Phase 2DTR-4 output is now the active hold.
- Review packet: `docs/mvp-review/phase-2dtr-4-exact-geometry-source-map-target-scene-spec/`.
- Proposed next task after Batu review: Phase 2DTR-5 - Exact Review Geometry Fixture To Raster Prompt Adapter.
- The next implementation batch must remain review-only and must not begin 2DTR-5, a true regenerated image pass, app/source edits, public interfaces, package/tooling changes, or production/public-readiness changes until the current brief explicitly opens that scope.
...
- Whether Batu accepts, revises, or rejects the Phase 2DTR-4 exact geometry source map and target scene spec.
...
- Whether the Phase 2DTR-4 output proves enough of the source-to-scene path to open the exact review geometry fixture and raster prompt adapter.
