...
- Current next task: none; pending Batu review of the completed 4M-R7 Franklin-only hero kit extraction/render proof.
- Pre-authorized queue: none.
- Hard stop gate: stop after completed 4M-R7 for Batu review. Do not open R8 or Manhattan. Do not start Mapillary/KartaView, 4L-External, production 4M asset/art-direction work, 4P, external source access, source download/cache/ingestion/conversion/render use, new evidence intake, normal-mode rendering, public UI, production scaffold rendering, source promotion, exact storefront/frontage/entrance/address/signage/tenant/material/active-status claims, exact height/roof claims, normal-mode exposure, production use, new dependencies, credentials, paid APIs, renderer replacement, public interfaces beyond the internal Franklin QA hero module, architecture changes, or public/product claims without Batu approval and an updated current brief/queue.
...
- Phase 4 execution gate: agents may execute only the current batch named in `docs/CURRENT_EXECUTION_BRIEF.md` or the next batch already named in that brief's pre-authorized queue, must use `docs/phase-4-execution-roadmap.md` as the operating plan, and must stop at hard Batu review gates instead of inventing, skipping, or opening unqueued batches. Current gate is the completed 4M-R7 Batu review stop with no executable batch and no pre-authorized queue.
...
85. Mapillary/KartaView source-use gate. Recommended as a proposal only; not open.
86. 4L external/scaled evidence-backed QA corridor render. Deferred until Batu approves source/evidence boundaries and opens a later packet.
87. 4M production asset registry / visual system / art-direction translation. Deferred until Batu opens production visual-system scope; Qwen/Oxen may be evaluated here only as a source-safe visual-system accelerator.
88. 4N: Normal-mode promotion + recognition QA. Deferred until QA render, spatial scaffold acceptance, provenance, optional asset/art translation, and Batu promotion gates clear.
...
- Batu review decision after 4M-R7: whether the extracted Franklin-only hero kit path preserves measured alignment, whether GLB/GLTF should become the next authoring path for benchmark fidelity, and whether a later Franklin benchmark asset batch should open. Manhattan remains closed.
