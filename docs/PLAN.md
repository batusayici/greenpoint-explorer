...
**Remaining Phase-3 order:** **3.15 (business cards) → 3.2 (camera) → 3.3 (all-angle) → 3.4 (lighting) → 3.5 (interaction generalize) → 3.6 (acceptance).** 3.15 is inserted to ship a business-facing feedback vehicle early.

**3.15 Hero Business Cards (feedback vehicle) — DO NEXT.** A business-facing demo: clicking a hero corner opens a paper II-C "place card" (tethered to the building by a pin) showing real, sourced business data for the three heroes (Premier/Franklin Organic, Sonny's, Sereneco), display-only, with an unofficial-prototype disclaimer. Purpose: let Batu start collecting feedback and ideas from local businesses while the rest of Phase 3 is built. This pulls forward and focuses the place-card half of the old 3.5 onto the three heroes with real data. Decisions (2026-06-15): build to the `II-B-place-card-marker-hover-state.png` reference with a **trimmed IA** (no Save/Share, no hours in v0); data via documented public-source research **proposed by agent, approved by Batu** before any demo (per `PLACE_SOURCE_POLICY.md`); display-only (no in-app submissions). Detailed plan: `docs/superpowers/plans/2026-06-15-hero-business-cards.md`. **Gate:** Batu approves the proposed business data before it is shown to anyone.

**3.2 Multi-angle camera rig (the enabler — after 3.15).** Four fixed iso rotation steps (90°) with animated snap, keeping pan/zoom; free-cam stays debug-only. Contained to `SceneView.jsx` (azimuth becomes one of four; pan axes + composition recompute per step). No geometry change. Existing hero textures live on world-space faces, so rotation views them correctly — it does not break the corner fold / kinks. **Acceptance:** rotate through all four; scene stays coherent and framed at each.
