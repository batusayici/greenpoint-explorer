...
- Current phase: MVP-22 Grillpoint / Greenpoint Ave G Real Corner Vertical Slice Stage A is approved-with-constraint; the Stage B implementation brief is prepared, and app integration has not started. MVP-21 remains accepted only as `approved-reviewable-limited-fallback`; MVP-17 remains accepted only as the product-facing raster interaction polish baseline, with the missing mobile selected-state containment screenshot recorded as an accepted evidence gap; MVP-15B remains visually rejected as product-facing primary world art.
- Current Codex task pointer: `docs/CURRENT_EXECUTION_BRIEF.md`.
- Current next recommended state: MVP-22 Stage B raster-first real-corner vertical slice is the next eligible task, but it is not started. Before implementation, use `docs/mvp-review/mvp-22-grillpoint-greenpoint-g-real-corner-vertical-slice/STAGE_B_IMPLEMENTATION_BRIEF.md`, which names the raster asset path, sign-label treatment, factual card copy and source URLs, allowed files, and screenshot QA requirements. Generic QA/demo freeze is not open yet.
...
| 28. Grillpoint / Greenpoint Ave G Real Corner Vertical Slice | Move from generalized fallback to one real-place art-directed vertical slice for the NW Grillpoint / Greenpoint Ave G context. | MVP-21 limited fallback is accepted; Batu opens the MVP-22 evidence/composition path. | Stage A source/reference packet verifies Grillpoint identity/address/sign and station-context relationship enough to propose a truthful composition; Stage B raster-first art-directed scene/app integration opens only after Batu accepts Stage A and names the raster asset path, sign-label treatment, factual card copy, file allowlist, and screenshot requirements. | Stage A approved-with-constraint; Stage B implementation brief prepared; no app integration performed. |
| 29. MVP QA And Demo Freeze | Verify MVP behavior, truth policy, visual containment, and demo readiness. | MVP feature slice, validation verdict, polish pass, and any accepted real-corner evidence/reference/translation/integration verdicts exist. | MVP QA checklist, smoke-check report, accepted exceptions, and demo-freeze notes are complete. | Future-only; generic QA/demo freeze is not the current next task. |
| 30. MVP Completion / Post-MVP Parking | Mark MVP complete or hold with explicit blockers, and keep expansion ideas parked. | MVP QA And Demo Freeze exits. | MVP completion note records final status; post-MVP ideas remain parked unless Batu promotes them. | Future-only. |

## Current Next Task

The current next pointer is documented in `docs/CURRENT_EXECUTION_BRIEF.md`:

- MVP-22 Grillpoint / Greenpoint Ave G Real Corner Vertical Slice Stage A is approved-with-constraint at `docs/mvp-review/mvp-22-grillpoint-greenpoint-g-real-corner-vertical-slice/README.md`.
- MVP-22 Stage A records that Grillpoint identity/address/sign are supportable for Stage B planning and that Greenpoint Ave G may be used as nearby/adjacent transit context.
- The Greenpoint Ave G station relationship remains adjacent/nearby transit context; do not claim the station entrance is directly in front of Grillpoint unless stronger field/photo evidence supports that exact relationship and Batu approves it.
- MVP-22 Stage B raster-first real-corner vertical slice is the next eligible implementation path, using `docs/mvp-review/mvp-22-grillpoint-greenpoint-g-real-corner-vertical-slice/STAGE_B_IMPLEMENTATION_BRIEF.md`; app integration has not started.
- Generic QA/demo freeze is not the current next task.
...
- No app integration, raster generation, visual polish, renderer work, new assets, scraping, live data pipeline, production/public-release claim, staging, or further commit is approved by this pointer until Batu explicitly opens MVP-22 Stage B.
...
- MVP-22 Stage B implementation is not started; it may open only from the prepared Stage B brief when Batu explicitly opens implementation.
...
- Whether Batu opens MVP-22 Stage B raster-first real-corner vertical slice implementation from the prepared brief.
- Whether source/app data later keeps generalized `Corner Deli`, uses `GRILLPOINT DELI` internally only, changes the underlying target id/name, chooses context-only/omitted treatment, or fictionalizes the target differently.
- Whether exact `GRILLPOINT DELI` sign text may appear in a review-only raster pass in a later explicitly approved task.
- Whether public card copy remains blocked, becomes review-only, or is fictionalized.
- Whether the proposed Greenpoint Ave G station treatment may be shown as adjacent/nearby context only, or whether any stronger exact relationship is approved from the field-reference evidence.
- Whether exact `GRILLPOINT DELI` sign text is approved for review-only raster art.
...
- Whether Batu supplies or approves the implementation-specific MVP-22 raster material path for the real-corner vertical slice.
- Whether a later brief should open limited QA/demo freeze refinements, accepted evidence-gap notes, optional ambient, or another next phase after MVP-22.
