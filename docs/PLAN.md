...
- Current next task: execute 4L-Local-3 inside the approved repo-local evidence-backed QA scene expansion packet.
- Pre-authorized queue: 4L-Local-4.
...
- Phase 4 execution gate: agents may execute only the current batch named in `docs/CURRENT_EXECUTION_BRIEF.md` or the next batch already named in that brief's pre-authorized queue, must use `docs/phase-4-execution-roadmap.md` as the operating plan, and must stop at hard Batu review gates instead of inventing, skipping, or opening unqueued batches. Current gate is 4L-Local-3 with 4L-Local-4 queued, then stop.
...
74. 4L-Local-3: QA runtime evidence overlay. Current executable batch.
75. 4L-Local-4: Visual review gate report. Queued; stop at Batu review gate.
