...
- Phase 4 execution gate: agents may execute only the current batch named in `docs/CURRENT_EXECUTION_BRIEF.md` or the next batch already named in that brief's pre-authorized queue, must use `docs/phase-4-execution-roadmap.md` as the operating plan, and must stop at hard Batu review gates instead of inventing, skipping, or opening unqueued batches. Current gate is the completed 4M-R5 Batu visual review stop with no executable batch and no queue.
...
83. Mapillary/KartaView source-use gate. Recommended as a proposal only; not open.
84. 4L external/scaled evidence-backed QA corridor render. Deferred until Batu approves source/evidence boundaries and opens a later packet.
85. 4M production asset registry / visual system / art-direction translation. Deferred until Batu opens production visual-system scope; Qwen/Oxen may be evaluated here only as a source-safe visual-system accelerator.
86. 4N: Normal-mode promotion + recognition QA. Deferred until QA render, spatial scaffold acceptance, provenance, optional asset/art translation, and Batu promotion gates clear.
...
- Batu visual review decision for the completed 4M-R5 Franklin hero corner fidelity layer: whether the Franklin/Manhattan endpoint output is close enough to the supplied evidence and benchmark fidelity target for proof-of-pipeline purposes, or whether to open another bounded endpoint correction/extraction pass.
