...
- Current next recommended state: MVP-20 is complete for Batu review with a `proceed-to-mvp-21` recommendation, and no next task is approved. MVP-21 One-Corner Raster Integration / Visual Pass is recommended only after Batu accepts MVP-20 and opens a tightly scoped implementation brief. No implementation phase, visual artifact generation, renderer work, new assets beyond supplied reference photos, screenshot work, scraping, live data pipeline, package/config/tooling change, staging, or commit is approved.
...
- Current blockers: no next implementation phase is approved; MVP-20 recommends a one-corner raster pass but leaves Batu decisions open for Greenpoint Deli vs `GRILLPOINT DELI`, exact sign text vs generalized treatment, source-data label changes, primary raster material, allowed source files, and screenshot/self-audit requirements. MVP-21 One-Corner Raster Integration / Visual Pass requires a separate current brief and approved implementation boundary. Production visual assets, production asset direction, production asset pipeline, real-place production cards, exact real facades, exact addresses, exact station geometry, final factual card copy, final architecture, live data, CI, deployment, backend/CMS/persistence/analytics, and broad map coverage remain blocked.
...
- MVP-20 Real-Corner Translation Boundary is complete for review at `docs/mvp-review/mvp-20-real-corner-translation-boundary/README.md`.
- MVP-20 recommendation is `proceed-to-mvp-21`: the supplied northwest corner references are strong enough to plan one recognizable raster translation, but Greenpoint Deli / `GRILLPOINT DELI`, exact sign treatment, source-data label changes, and allowed implementation files remain Batu decisions.
- No next task is approved. MVP-21 may be opened only by Batu with a separate current brief and approved implementation boundaries.
- MVP-21 One-Corner Raster Integration / Visual Pass, if later opened, must remain tightly scoped to the selected NW corner and cannot authorize production assets, exact facade claims, exact frontage/order claims, exact station-geometry claims, or public-release card copy.
...
- Whether Batu accepts, revises, or rejects the MVP-20 `proceed-to-mvp-21` recommendation.
- Whether Batu keeps northwest Greenpoint Deli / 903 Manhattan Ave as the target, corrects the target label to `GRILLPOINT DELI`, chooses a fallback, or fictionalizes the target.
...
- Whether Batu opens MVP-21 One-Corner Raster Integration / Visual Pass and which files it may touch.
