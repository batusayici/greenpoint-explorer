...
- Current next task: execute 4L-Local-2 inside the approved repo-local evidence-backed QA scene expansion packet.
- Pre-authorized queue: 4L-Local-3, then 4L-Local-4.
...
- Phase 4 execution gate: agents may execute only the current batch named in `docs/CURRENT_EXECUTION_BRIEF.md` or the next batch already named in that brief's pre-authorized queue, must use `docs/phase-4-execution-roadmap.md` as the operating plan, and must stop at hard Batu review gates instead of inventing, skipping, or opening unqueued batches. Current gate is 4L-Local-2 with 4L-Local-3 and 4L-Local-4 queued, then stop.
...
73. 4L-Local-2: Evidence-backed QA cue enrichment. Current executable batch.
74. 4L-Local-3: QA runtime evidence overlay. Queued.
