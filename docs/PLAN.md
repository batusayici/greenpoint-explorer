...
- **8.1 Expand along the story-dense spine — DONE (2026-06-21).** Extended the procedural scene up the curated Franklin corridor. Done in two steps:
  ...
  - **Scope decision (Batu, 2026-06-21):** kit applied to `brick`/`brownstone`/`clapboard`; `brownstone`/`clapboard` only via evidence-bound overrides (none in this corridor yet → all-brick); `painted-masonry`/`warehouse`/`modern-flat` left as-is. Spec/plan in `docs/superpowers/`.
