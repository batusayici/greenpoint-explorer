...
- **8.0 Structural Depth Pass — geometry DONE + APPROVED, look gated on 2 craft follow-ups (2026-06-21).** Real extruded depth (3D stoops + front fire escapes), parametric and family/storey-gated, built + tested + verified on the 4-BIN pilot (pure modules `facadeDepthGates.js`/`stoopGeometry.js`/`fireEscapeGeometry.js` + `decorateInkedWall` wiring; 136 tests green; commits `905315f..f9ba537`). Batu approved the geometry at the Task 6 gate; design + plan in `docs/superpowers/`, decisions in `DECISION_LOG.md` (2026-06-21). **Two follow-ups gate the look (and therefore gate 8.1 fan-out):** (a) regenerate brick + brownstone ground textures without painted stairs/door (`task_f39b0155`) so the painted ground-floor band and the 3D stoop coexist — currently they double up (double-stairs on brownstone); (b) open up the fire-escape ironwork (alpha-textured open railings) before locking the relief-vs-lattice variant — opaque-quad rails currently read as solid dark shelves, not iron. Basement/areaway = **Phase 8.5**, its own ref-gated mini-design pending Batu photos.
- **8.1 Expand along the story-dense spine** — extend the procedural scene down the curated corridor (not all of Greenpoint). Hero treatment only on the 6.1 set; everything else typological through the completed kit. **BLOCKED on 8.0's two craft follow-ups + look re-gate.**
