...
- **8.1 Expand along the story-dense spine — DONE (2026-06-21).** Extended the procedural scene up the curated Franklin corridor. Done in two steps:
  ...
  - **Evidence-backed material showcases (Batu-confirmed, 2026-06-21).** Restored visible brownstone + clapboard after the flip reverted the unverified pilots to brick: **95 Kent St (3064605) → clapboard**; **168 Franklin St (3064541) → dual-material** (brownstone ground floor, light brick above). The dual-material build added a `groundFamily`/`groundTint` override (kit now renders the ground floor — band/stoop/door — in a distinct material from the wall above), shipping the "dual-material" variation from the facade-variation-layer plan. 151 tests + conformance green. **Visual-verify gap:** the brownstone base band's on-screen pixels on 168 Franklin were NOT framed in-engine (camera held in a React closure; the bin-tagged runtime mesh is a degenerate marker) — data/params verified by Node probe, the render pixels are pending a Batu glance or a dev-isolation pass.
