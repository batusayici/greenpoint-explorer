...
- Current next recommended state: MVP-21 One-Corner Raster Integration / Visual Pass implementation is recommended only after Batu approves the MVP-21 brief, chooses the NW label/sign treatment, approves or supplies primary raster material, and confirms the allowed file list. No implementation phase, visual artifact generation, renderer work, new assets beyond supplied reference photos, screenshot work, scraping, live data pipeline, package/config/tooling change, staging, or commit is approved.
...
- Current blockers: no next implementation phase is approved; the MVP-21 brief leaves Batu decisions open for Greenpoint Deli vs `GRILLPOINT DELI` vs generalized/fictional/context-only/omitted label treatment, exact sign text vs generalized treatment, source-data label changes, primary raster material, allowed source files, and screenshot/self-audit requirements. MVP-21 One-Corner Raster Integration / Visual Pass requires explicit Batu approval before source edits. Production visual assets, production asset direction, production asset pipeline, real-place production cards, exact real facades, exact addresses, exact station geometry, final factual card copy, final architecture, live data, CI, deployment, backend/CMS/persistence/analytics, and broad map coverage remain blocked.
...
| 27. One-Corner Raster Integration / Visual Pass | Integrate an approved/supplied raster plate, layered raster export, or approved raster sprite/asset-kit composition for one selected real-corner target while preserving interaction shell and truth-safe UI/card behavior. | MVP-18 supports the selected corner; MVP-19 provides owned/approved non-Google storefront/facade references or an approved equivalent; MVP-20 is accepted by Batu; MVP-21 implementation brief is approved by Batu; a later current brief explicitly opens implementation; public interfaces/module boundaries and allowed files are documented and reviewed. | One-corner raster integration is reviewable; interaction shell/card/target behavior is preserved; real-place placement and card behavior remain tied to the truth layer; product-facing world surface remains raster-first; no code-native buildings/storefronts/roads/signs/props/textures become primary world art; no production asset/data, exact facade, exact address, exact frontage/order, or exact station-geometry claim is made; screenshots/self-audit/checklist exist only if the current brief explicitly requires them. | Brief complete for review as MVP-21; implementation blocked until Batu approval and primary raster material decision. |
...
- MVP-21 One-Corner Raster Integration / Visual Pass Brief is complete for review at `docs/mvp-review/mvp-21-one-corner-raster-integration-brief/README.md`.
- MVP-21 implementation is recommended but still unopened: Batu must approve the brief, choose the NW label/sign treatment, approve or supply primary raster material, and confirm the source-file allowlist before source edits.
...
- If no approved primary raster material exists at the named path, Codex must stop before app/source edits.
...
- MVP-21 One-Corner Raster Integration / Visual Pass implementation requires explicit Batu approval of the brief, label/sign treatment, primary raster material, and allowed file list.
...
- Whether Batu accepts, revises, or rejects the MVP-21 implementation brief.
- Whether Batu keeps northwest Greenpoint Deli / 903 Manhattan Ave as the target, corrects the target label to `GRILLPOINT DELI`, generalizes it, chooses context-only/omitted treatment, or fictionalizes the target.
- Whether exact `GRILLPOINT DELI` sign text may appear in a review-only raster pass.
