...
- Current next task: none. R10 is a future candidate only, pending Batu approval after R9 visual review.
- Pre-authorized queue: none.
- Hard stop gate: stop after completed 4M-R9 for Batu visual review. Do not open R10, GLB work, Manhattan implementation, Mapillary/KartaView, 4L-External, production 4M asset/art-direction work, 4P, external source access, source download/cache/ingestion/conversion/render use, new evidence intake, normal-mode rendering, public UI, production scaffold rendering, source promotion, exact storefront/frontage/entrance/address/signage/tenant/material/active-status claims, exact height/roof claims, production use, new dependencies, credentials, paid APIs, renderer replacement, public interfaces beyond the internal Franklin QA hero module / candidate hero asset boundary, architecture changes, or public/product claims without Batu approval and an updated current brief/queue.
...
- Phase 4 execution gate: agents may execute only the current batch named in `docs/CURRENT_EXECUTION_BRIEF.md` or the next batch already named in that brief's pre-authorized queue, must use `docs/phase-4-execution-roadmap.md` as the operating plan, and must stop at hard Batu review gates instead of inventing, skipping, or opening unqueued batches. Current gate is completed 4M-R9 with no executable batch, no pre-authorized queue, and a Batu visual review stop; R10/R11/R12 are planned future candidates only.
...
87. 4M-R10: Franklin hero asset ingestion spike. Future candidate only after Batu opens it; one QA-only GLB insert, structured record anchoring, procedural fallback, and Batu visual review stop.
...
- Blender/GLB assets are deferred optional enhancement/override layers after the truth-first scaffold. The post-R9 candidate path narrows that idea to one QA-only Franklin hero insert first: the procedural scaffold remains the base; QA-only hero inserts are exceptions for high-recognition architectural features.
...
- Batu visual review decision after 4M-R9: whether to open 4M-R10 as a controlled one-asset Franklin hero ingestion spike to test recognizable stylized fidelity under close isometric orthographic review. Manhattan remains closed except regression capture if shared renderer code changes.
