...
- Current next recommended state: MVP-21 One-Corner Raster Integration / Visual Pass implementation is recommended only after Batu explicitly opens implementation, confirms the implementation raster material path, and confirms the allowed file list. Batu has named `docs/archive/visual-artifacts/batch-13-survivor-direction-development/generated/II-A-ui-world-integration.png` as the primary raster/world integration reference and `docs/archive/visual-artifacts/batch-13-survivor-direction-development/generated/II-B-place-card-marker-hover-state.png` as the UI/card/marker/hover-state reference. No implementation phase, visual artifact generation, renderer work, new assets beyond supplied reference photos, screenshot work, scraping, live data pipeline, package/config/tooling change, staging, or commit is approved.
...
- Current blockers: no next implementation phase is approved; Batu has approved `GRILLPOINT DELI` as an internal/source evidence label only and prefers generalized/fictional-safe product-facing deli treatment such as a non-readable deli sign or `Corner Deli`; exact `GRILLPOINT DELI` sign reproduction is not approved unless explicitly approved later. MVP-21 One-Corner Raster Integration / Visual Pass requires explicit Batu approval before source edits, plus confirmation of the implementation raster material path, allowed source files, and screenshot/self-audit requirements. Production visual assets, production asset direction, production asset pipeline, real-place production cards, exact real facades, exact addresses, exact station geometry, final factual card copy, final architecture, live data, CI, deployment, backend/CMS/persistence/analytics, and broad map coverage remain blocked.
...
- MVP-21 One-Corner Raster Integration / Visual Pass Brief is finalized for review at `docs/mvp-review/mvp-21-one-corner-raster-integration-brief/README.md`.
- Batu has named the exact review-only raster references for MVP-21: `docs/archive/visual-artifacts/batch-13-survivor-direction-development/generated/II-A-ui-world-integration.png` for primary raster/world integration and `docs/archive/visual-artifacts/batch-13-survivor-direction-development/generated/II-B-place-card-marker-hover-state.png` for UI/card/marker/hover-state treatment.
- MVP-21 implementation is recommended but still unopened: Batu must explicitly open implementation and confirm the implementation raster material path and source-file allowlist before source edits.
- MVP-21 One-Corner Raster Integration / Visual Pass, if opened, must remain tightly scoped to the selected NW corner and cannot authorize production assets, exact facade claims, exact frontage/order claims, exact station-geometry claims, or public-release card copy.
- If no approved implementation raster material exists at the named path, Codex must stop before app/source edits.
...
- MVP-21 One-Corner Raster Integration / Visual Pass implementation requires explicit Batu approval to open implementation, confirmation of the implementation raster material path, and confirmation of the allowed file list.
...
- Whether Batu opens MVP-21 implementation.
- Whether source/app data keeps Greenpoint Deli, uses `GRILLPOINT DELI` internally only, generalizes it, chooses context-only/omitted treatment, or fictionalizes the target.
...
- Whether Batu opens MVP-21 One-Corner Raster Integration / Visual Pass implementation and which files it may touch.
...
- Which implementation raster material path, if different from the approved raster references, is allowed for MVP-21.
