...
- **6.1 Hero + Landmark tiering (one pass) — APPROVED 2026-06-18.** A single curation producing two views of the same set: the **hero visual tier** and the **landmark story-object tier**. Output landed in **`docs/CURATION_TIERS.md`** (the authoritative list): 11 v1 heroes (within the ~10–12 budget), the typological default, the landmark spine, and locked decisions (Black Rabbit + Brouwerij Lane + Eberhard Faber building promoted to hero; Franklin-north spine first; Manhattan Ave a later node). Remaining 6.1 execution: tag built buildings with their `tier` in data/code; fix the stale "Pencil Factory" (closed bar) roster record; add Brouwerij Lane data.
