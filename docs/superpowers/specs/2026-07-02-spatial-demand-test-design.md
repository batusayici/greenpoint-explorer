...
Status: **v1 live (2026-07-02, updated 2026-07-03) — main (PR #4), deployed: <https://greenpoint-explorer.vercel.app/july.html>.** 34 cards (8 discovery · 22 events — refreshed 2026-07-08 to the Greenpointers Jul 7–12 week, 10 past events dropped · 1 subscription · 3 G-train campaign/action cards). Plan: `docs/superpowers/plans/2026-07-02-track-v-spatial-demand-test.md`.
**2026-07-03 moat-visibility upgrade** (per the differentiation doc review — make "structure behind the pins" experienceable, not just carried in schema): `g-train-closures` campaign card w/ rendered source-backed timeline · related-card chips rendered + reciprocal G-train/World-Cup graph links (`related_tap` instrumented) · tappability rule — every action is url / share / internal filter-switch (`action.filterId`) / directions derived from the card's own address (no dead action spans; dead rsvp/order/gift-card labels cut or folded into summaries) · source publishers tappable when a URL exists (`source_tap`) · all 8 new businesses + Falu tagged `g_train` ("still open this weekend") · reader copy spells out Shop Small Greenpoint.
**Next (plan review, Batu + agent, 2026-07-03):** 1) instrumentation — DONE + VERIFIED IN PROD 2026-07-03 (named tap events via trackEvents.js + Vercel Web Analytics; live smoke test: `/_vercel/insights/event` POSTs return 200, so custom events record — no plan gating) · 2) forms — DONE 2026-07-03 (single-CTA funnel per `3455063`: one ask, business/event intake as an optional field; live Tally form `tally.so/r/44daZo` wired in `b95b9ff`; measurement note: pin_tap + card_open together = discovery interest; cta_tap counts intent, Tally responses count completions, intake-field responses count the "businesses ask in" signal) · 3) schema catch-up — DONE 2026-07-03 (relatedCardIds/timeline/trustRisk in cardSchema.js, sparse seed links) · 4) factual review + Jul-10-weekend refresh · 5) distribute to cohort + Perri walkthrough over Jul 10–13 closure; **go/no-go review ~Jul 15**.
