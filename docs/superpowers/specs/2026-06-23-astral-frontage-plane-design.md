...
3. **Register Astral + wire the center segment — DONE (placement, 2026-06-23).** `FACADE_GROUP_BINS["3064408"]="astral-apartments"` (promotion 1b resolves it), `II_PALETTE.heroes` hue `0x9e4b35`, one centered `franklin-center` segment (fromM 21.35 / toM 39.25 of the real 60.6m chord). `buildHeroBuilding` builds the frontage-chord plane via `frontagePlane.frontageChord` (street-most band: refined to reject Astral's interior light courts that drag the plane back) + `segmentURange`, places the textured segment proud of the typological frontage, culls per-view. **Verified in-engine** at all four angles: segment lands centered on the real Franklin frontage, correct ~17.9m width (matches texture aspect, no distortion), full height; no blank box / see-through / z-fight. 220 tests + full `npm run verify` green. **Hand-seeded recesses — DONE (first pass, 2026-06-23).** Auto-derive failed, so openings were read off the render by hand into `src/data/facade-specs/astral-apartments.v0.1.json` (face `3064408:frontage`): 5 window columns × 5 floors, round-arch entrance + top-floor arcade + giant center bay tagged `shape:"arch"`. The segment now routes through `buildFacadeAssembly` (same rebuild + `registerFacadeFace` machinery as per-edge specs) so recesses carve AND `?facadeedit=1` snaps them live. Verified in-engine: recesses carve with depth, arches render; **alignment is approximate by design — refine in the recess editor.** **Remaining:** editor-refine opening positions vs the render, confirm centering vs IMG_0971, flank renders + oriels, flip `building-tiers` buildStatus→built + update `curationTiers.test.mjs`.
