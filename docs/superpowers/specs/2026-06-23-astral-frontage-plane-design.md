...
3. **Register Astral + wire the center segment:** `FACADE_GROUP_BINS["3064408"]="astral-apartments"` (promotion 1b makes this resolve), composite with one `franklin-center` segment placed on the frontage chord via `frontagePlane.js`, `II_PALETTE.heroes` hue, hand-authored arched recesses.
