...
detail, partner assessments, and the sourced market evidence — lives outside the
repo by decision (2026-07-28) at `docs/private/business-model.md`, which is
gitignored and never committed.

Nothing here is a number, and that is deliberate: agents and cloud routines need
the *constraints* to avoid violating the model. They do not need the economics.
**If a task seems to require the numbers, ask Batu rather than inferring them.**

Adopted by Batu 2026-07-28 after a blank-slate re-evaluation; **supersedes the
claim model** (storefront signs default to category labels, businesses pay to
attach real branding), which belonged to the parked 3D concept and lost its
premise once the product became automatically-sourced verified events from
verified local businesses. Decision: `DECISION_LOG.md` 2026-07-28.

Companion to `growth-engine.md` (how the audience compounds); §5 there is the seam.
...
These extend the truth rules into money. They are not tradable for revenue, and
they hold at every stage:

1. **Residents never pay.** No paywall, no premium tier, no supporter ask, no
   feature held back for money.
...
5. **Hard caps on paid density** — at most one featured slot per lens per week.
   The feed must never read as an ad channel.
6. **No payer influences coverage.** Not what ships, not what's omitted, not tone.
...
the last — **expansion is a revenue event, not a cost event.**

The standing editorial rule is untouched: **do not build "Williamsburg North"** —
never dilute Greenpoint's product with adjacent-neighborhood content. Serving
another neighborhood means a properly scoped surface of its own.
...
| Prior | Now |
|---|---|
| Claim model (category labels → pay for branding) | **Retired.** Premise gone: the product sources verified businesses automatically. |
| "Never charge small businesses first" | **Sharpened:** never sell *coverage*; sell function/presence. Institutions before small businesses **in time** (Layer 1 → Layer 2). |
