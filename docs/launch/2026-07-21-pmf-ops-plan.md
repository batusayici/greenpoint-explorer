...
- 3.3 Business submission path — **shipped 2026-07-28** (DECISION_LOG entry): "Add your event — free, verified, on the map" as a quiet feed-end row + empty-state echo (chrome, not a pinned card), linking an ultra-light Tally form (name · what's happening · email; review-ready fields are a data-gated upgrade). `submit_tap` live; submissions join the Monday review queue as "asks" (ingest-skill §1). Supply-side PMF sensor. Remaining: Batu creates the dedicated form and the URL/env id get swapped.
