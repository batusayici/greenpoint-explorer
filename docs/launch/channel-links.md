...
|---|---|---|---|
| Weekly digest — the Greenpoint edition (R1 control arm) | `digest` | https://stoopwise.com/?src=digest | **new 2026-07-28 · Thursday 7am from Sep 17 (2026-09-13)** — goes to everyone who followed Greenpoint rather than a lens, plus any lens under 5 subscribers. Composed as a fresh Gmail draft by the operator, sent by Batu; picks link `https://stoopwise.com/e/<slug>?src=digest`. **No email was ever sent before 2026-09-17** (learning log L2026-09-13) — the two "sends" the note below describes did not happen; the `digest\`` arrivals were fetches of the link as printed in readouts. Kept for the copy discipline it records: ⚠ **the trailing backtick cost two drafts** — 7 of 15 arrivals on 2026-08-25 (47%) and 4 of 13 on 2026-09-01 (31%), all landing on an orphan ``digest` `` tag that matches no row here. The 8/25 link was copied out of a readout draft that wrapped it in code formatting; the 9/1 send repeated it because it was composed from the 8/25 email, which still carried the bad URL. **Copy the cell above and nothing around it, and compose each weekly note as a NEW message — never a reply or forward of the previous one**, or the broken link travels with it. |
| **Follow — Family & Kids (R1 treatment) — the Family & Kids edition of the weekly digest** | `follow-family-kids` | https://stoopwise.com/kids?src=follow-family-kids | **new 2026-08-03 · link changed 2026-09-13** to the `/kids` page, which exists since 2026-09-10 and lands readers on the Family & Kids view. Picks inside the email link their own card pages as `https://stoopwise.com/e/<slug>?src=follow-family-kids` — `src` is read from any URL's query string, so a card link is attributed the same way. First send Thu Sep 17, 7am. Template and checklist: `docs/growth/digest.md`. |
...
| **Q2 groups — North Brooklyn Civic Crowdsource (Rana)** | `civic-crowdsource` | https://stoopwise.com/civic?src=civic-crowdsource | **new 2026-09-10 · link changed 2026-09-11 · bold markers removed 2026-09-15** (see the plain-text rule below — an orphan ``civic-crowdsource` `` tag appeared on 9/11). Rana names this group and Greenpoint Sharing as her two that are not about kids, so the `parents` link is wrong for both — it lands readers on the Family & Kids view. This one gets its own row and the `civic` lens: the group is about the neighborhood's civic questions, which is the layer that answers them. `/civic` is a real page as of 2026-09-11, so it previews as *"Community meetings and volunteering in Greenpoint, Brooklyn"* rather than the generic site headline. |
| **Q2 groups — Greenpoint Sharing (Rana)** | `gp-sharing` | https://stoopwise.com/?src=gp-sharing | **new 2026-09-10 · bold markers removed 2026-09-15** (an orphan ``gp-sharing` `` tag appeared on 9/10). No lens — a sharing group has no one layer, so the general feed is the offer. **Check the group's posting rules before Rana sends:** several Greenpoint sharing and Buy Nothing groups ban outside links outright (Buy Nothing rule 8 is already noted on the `fbgroups` row), and neither Rana nor this table has confirmed this one's. If links are barred, this is an answer-post channel like `fbgroups`, not a link drop. No lens page for the same reason there is no lens: this one previews as the general site headline, which is correct for it. |
...
| Home-screen install | `homescreen` | *(generated by the product)* | **product-generated — never send by hand. Row added 2026-09-15; the tag has existed since 2026-08-12.** `public/site.webmanifest` sets `"start_url": "/?src=homescreen"`, so a reader who adds the map to their home screen is attributable every time they open it from there. First and so far only install: 2026-09-12, which came back across five sessions and two days. It was missing from this table for a month, which is why the rule below now says an unlisted tag is a defect wherever it comes from, product-generated included. |
...
- **The Link column is plain text. No bold, no backticks, no code formatting,
  ever** (2026-09-15). A delimiter next to a URL travels with the copy: the
  trailing backtick that cost 47% and then 31% of the digest's arrivals in
  August came back three more times on 9/10–9/11, once each on `parents`,
  `civic-crowdsource` and `gp-sharing`, the two days those rows were written
  with their links in `**bold**`. The markers are gone from all three rows. If a
  link is copied from anywhere other than this table — a chat message, a note,
  last week's email — that source needs the same treatment, because this file
  cannot protect a copy it never made.
- **Every `src` that can appear in the data has a row here, including tags the
  product generates itself.** `homescreen` ran for a month without one. An
  unlisted tag is a defect, not a curiosity: the weekly readout has to be able
  to tell an orphan tag apart from a channel nobody wrote down.
