...
| Q2 parents — one row per group | `parents-<group-slug>` | https://stoopwise.com/?src=parents-&lt;group-slug&gt; | **scheme set 2026-08-25.** Rana posts to **multiple** groups in her own words; Batu supplies the list closer to Sep 8 and each group gets its own row here **before copy is finalized**. Q2's decision reads the **pooled** `parents-*` family — per-group rows are diagnostics, because a single group reads n≈3. Guidance for the list: Williamsburg & Greenpoint BK Parents was measured at **0 posts/month, dormant** (8/17) and is not worth one of her posts. |
