...
- **Tue–Wed 8/18–19 mid-morning:** Library + Brooklyn Craft notes (final
  drafts in the roster, share lines included; preflight fills the Library
  count or the bracket is dropped).
- **Tue 8/18:** readout fires mid-wave — P6 no-read instruction applies.
- **This week, not "before Q2": FB in-app webview test on real iOS + Android**
...
- **Send-ready drafts** written into the seeding roster (Town Square
  re-centered for Monday; Library + Brooklyn Craft gain the share line);
  superseded markers added to the older drafts.
- **Per-category feed-density cut** added to growth-engine §1 and the
  growth-weekly readout template: cards + dated-in-window by filter. This is
  the instrument that would have surfaced the civic gap (5 cards, 1 dated) and
  any September kids thinning without a manual sweep.
- **Doc drift:** growth-engine §0 no longer claims the Follow CTA "has no
  in-product surface" (it shipped 2026-07-29); PLAN.md no longer calls R1
  "weekly digest" (digest is the control arm); the launch plan's "R1 first
  Monday post-launch" carries its superseded marker (P5).

## Tuesday proposal queue (deliberately not in this package)

Routed through the normal readout cycle, one per week at most:

1. **Correction-SLA coverage:** auto-acknowledgement on the correction form;
   daily ingest routine may unpublish a card named in a dispute (unpublish-first
   is already doctrine, deletion already pre-approved 2026-07-16); one
   "disputes: N, SLA met: Y/N" readout line. Today the <24h promise rides
   entirely on Batu's inbox and nothing measures it.
2. **Loop B ignition floor + instrument reconciliation:** define "engagement
   clears the signal floor" for the first proof-of-value email (proposal: first
   business whose cards reach N combined high-intent acts), and add a Monday
   line reconciling `submit_tap` count against Tally submissions — a broken
   form must not read as "no supply" forever (the L10 precedent).
3. **Neighborhood WhatsApp groups** added to the Wave-2 channel list (own src
   row when played) — group chats are where 2026 sharing actually happens.
4. **Civic supply push:** CB1 block-association roster, the two community
   gardens, Church of the Ascension — civic is 5 cards / 1 dated against the
   business model's differentiator #3.
5. **New-resident framing** named as an explicit goal of the Wave-2 Reddit
