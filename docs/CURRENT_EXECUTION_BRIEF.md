...
- Foursquare credential/export/response fixture path plus usage/caching/display approval. The prepared Foursquare contract lives at `docs/phase-3-foursquare-brouwerij-poi-adapter-contract.md`, with stub script `scripts/prepare-foursquare-brouwerij-poi-evidence.mjs`.
...
- The Foursquare one-target adapter contract is `docs/phase-3-foursquare-brouwerij-poi-adapter-contract.md`; it defines the raw fixture path, normalized evidence output path, required env vars, allowed fields, cache/hash policy, and review-only status.
- The Foursquare credential/source blocker report is `docs/phase-3-brouwerij-foursquare-credential-blocker.md`; it records that no credential or terms/cache/display approval was present, so no API call, raw fixture, or normalized evidence output was created.
