# Current Execution Brief - Phase 2DTR-4 Review Hold

Status: Phase 2DTR-4 - Exact Geometry Source Map + Target Scene Spec is complete for Batu review. The next executable task is pending Batu approval or revision direction.
...
- Accept, revise, or reject the Phase 2DTR-4 exact geometry source map and target scene spec.

Review packet:

- `docs/mvp-review/phase-2dtr-4-exact-geometry-source-map-target-scene-spec/README.md`
...
- `docs/mvp-review/phase-2dtr-4-exact-geometry-source-map-target-scene-spec/generated/exact-geometry-source-map-board.png`

No Phase 2DTR-5 implementation, app/source edit, public-interface/schema approval, package/tooling change, true regenerated image pass, normal-mode raster replacement, or production/public-readiness change is authorized until Batu updates this brief or gives explicit direction.
...
- Move the exact review placement requirements into a review-only structured scene fixture.
- Generate a deterministic raster prompt and provenance map from that fixture.
- Add or run a verifier that fails if a visible scene field lacks `sourceCategory`, `status`, and source path.
...
## Verification For Phase 2DTR-4

- JSON parse for Phase 2DTR-4 generated artifacts.
- Raster file/dimension inspection for the source-map board.
- Visual inspection of the source-map board.
- `git diff --check`
- `git status --short`
- `git diff --stat`
