...
**Same day, second pass.** Two more rulings once the first batch came back.

**Batu may supply a fact the poster lost, and the card ships.** The Calyer Street block-party sheet was sun-bleached past its month; it read only "SATURDAY 12TH", which matches four different dates in the last two years, so the run held it. Batu supplied September, from his own knowledge of the block, and the card shipped as 9/12. This is the same shape as the Batu-supplied venue fact already on the `macha-studio` roster entry: he is a named source for what he knows about his own neighborhood, the fact is written into the poster's evidence file and into `watchItems` as permanently unverifiable by fetch, and every other claim on the card still needs its own quote. A future run must not "fix" this by inventing a source. **A guessed month is still not a date** — the hold was right, and what unblocked it was a person, not an inference.

**RESOLVED the same day: the map carries a card with no pin.** Batu's call, after the question below was put to him. A card may set `locationPrivate: true`, which drops the coords requirement: it rides the feed, drops no marker, and says on its own row and in its crawlable prose that the address comes with the RSVP. The flag is a claim about the SOURCE, never a shortcut — the schema rejects it alongside coords, an address or venues, so a run cannot mark a card private and pin it anyway, and a card that merely lacks coords still fails. Five Light & Sound nights shipped on it immediately (two Quadraphonics sittings on 9/12, then 9/13, 9/18, 9/19). The trade Batu accepted: the map is this product's container, and a feed-only card bends that — but the reader still gets the event, which is the point, and the alternative was carding nothing from an active neighborhood venue. **Reading the flyers also caught what the index could not:** the 9/20 "Last Sun Day" on L&SD's own calendar is at Rockaway Beach, stated nowhere but the image.

**The question that produced it —** **A venue that hides its address cannot be carded, and we do not out it.** Light & Sound Design Studios fetches clean and carries the densest music calendar the poster batch found — Present Sounds on Thursdays, dance nights, one-off bills, ten dated events in three weeks. Every one is uncardable: the venue deliberately does not publish where it is ("RSVP FOR LOCATION" on its poster, "L&SD (address with rsvp)" on its own Luma page), and the schema needs a pin. The address is findable on third-party listings; **publishing it is off the table** — the venue chose not to, and this product does not out a neighbor to fill a lens. The source is on the roster fetching, with `standing` unset so it does not flag as a dark source every run, and a `watchItems` entry holding the open question: can the map carry an event whose location is deliberately private? That is a product decision, not a supply problem.
