...
Encoded in `cardSchema.js` (`sourceQuote`), `julyCards.test.mjs` (dated substantiation test), `.claude/skills/ingest-newsletters/SKILL.md` (§1a extraction contract, §2 truth rules, §3 triage, §4 ship), `CLAUDE.md`, `AGENTS.md`, `README.md`, `docs/PLAN.md`, `docs/growth/growth-engine.md` (§7 ladder — content ingest at V3). **The three claude.ai cloud routines carry their own copies of the prompt and must be updated in the claude.ai UI — Batu's action; until then they will keep opening PRs.**
