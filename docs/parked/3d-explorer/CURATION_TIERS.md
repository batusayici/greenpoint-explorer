...
| 7 | Brouwerij Lane | 78 Greenpoint Ave, Brooklyn, NY 11222 | tier2 | **data missing → add** | Hero; craft-beer shop est. 2009, across from the Pencil Factory bldg. **Verified open (2026-06-18)** — needs scene geometry/roster entry in Phase 8 |
...
- **Brouwerij Lane — verified, awaiting geometry.** Hero #7, **78 Greenpoint Ave** (across from the Pencil Factory building). `verificationStatus:verified` (open as of 2026-06-18; official site + Yelp cited in the registry). `buildStatus:data-missing` only because it has no scene geometry yet — add roster/geometry entry when the Franklin-north spine reaches it (Phase 8).
