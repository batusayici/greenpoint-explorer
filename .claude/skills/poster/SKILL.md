...
- **Roster assessment.** Does this venue publish a calendar a script could read, and does it program Greenpoint events regularly? If yes, propose it in the PR against the `ingest-sources.json` shape (prefer `feed`/`json` over `browser`, per the ingest skill's fetch-strategy rule). A venue whose schedule lives only on Instagram goes in `manualSources` with the measurement that settled it, so a later run does not re-onboard it. **Roster additions stay human-gated** — propose, never merge them yourself.

**If a venue deliberately withholds its address, do not publish it.** Light & Sound Design Studios says "RSVP for location" on its poster and "L&SD (address with rsvp)" on its own ticketing page; the address is findable on third-party listings and is off limits anyway. The venue chose not to publish it, and this product does not out a neighbor to fill a lens. Such a source can sit on the roster fetching, with `standing` unset, until the map can carry an event without a pin.
