...
2. `npm run ingest:fetch` — snapshots every roster source and writes `.ingest-cache/changes.json`. First Monday of the month: add `--include-monthly`.
   ...
   **A non-zero exit means the roster was not readable — STOP, do not ingest (2026-08-03).** The script now preflights the browser path and exits 1 when errored sources exceed 15% or when every browser fetch failed. Read the `BROWSER PREFLIGHT` block: it names one of `playwright-missing`, `chromium-binary-missing`, `browser-egress-blocked` or `chromium-launch-failed`, each with its fix. **`browser-egress-blocked` is an environment fix Batu must apply** (allowlist outbound HTTPS for the routine at claude.ai/code) — report it and stop; never route around it.
