...
- Filters (2026-07-25 IA re-cut — lenses are a person's question, not a content taxonomy): author from `FILTER_IDS` in `cardSchema.js` (`new`, `food_drink`, `shopping`, `arts_culture`, `family_kids`, `live_music`, `wellness`, `community`, `deals_memberships`, `news`). There is no `events` umbrella — the day-grouped All feed is that answer. `wellness` = movement/mind-body (yoga, pilates, dance — NOT civic action); `community` = civic/mutual-aid stewardship (park cleanups, harbor day, adoption events, cleanup clubs, advocacy launches, stoop sales); civic *news* (closures, zoning, campaigns) still sits inside `news` — `community` is for hands-on participation, not reporting. Astrology/talks/science-adjacent programming goes to `arts_culture`. Deals AND subscriptions/memberships both go to `deals_memberships` unless the card is really civic (like a cleanup club — that's `community`, not a membership). A one-off with no honest lens gets an **empty** `filters: []` (All-only) — never force-fit; if All-only cards cluster (see the lens-less guard test in `julyCards.test.mjs`), flag the cluster as a candidate lens in the review diff. Retired ids (`events`, `services`, `deals`, `clubs_signups`, `g_train`) must never reappear — a guard test enforces this.
