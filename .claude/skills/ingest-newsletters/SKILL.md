...
3. Read `changes.json` (the report only — not the snapshots). Sources with status `unchanged` are DONE — do not open them, do not "double-check" them. **ONE EXCEPTION, and it is a big one (2026-08-07): a source marked `standing: true` in the roster.**
