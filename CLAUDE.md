...
npm run verify:aeo            # what an AI CRAWLER gets from dist/ — prose per card page, required JSON-LD fields per type, sitemap parity, single canonical, llms.txt links resolve, and dead /e/ links fall through to 404.html instead of serving the home page (2026-09-07). Needs a fresh build.
...
npm run ingest:pull      # cloud routine only: bring the fetched bundle into .ingest-cache/bundle/ (halts if >6h old)
...
4. **It does not refuse automated readers — and a publisher that blocks AI crawlers is a NO, not a question for Batu (his ruling, 2026-09-10).** Check `robots.txt` and any stated terms before adding. Our fetcher sends a Chrome user-agent, so a site that names `anthropic-ai` in a block list will let it through; that is not consent, it is a gap in their enforcement. Brooklyn Paper is the worked example — technically clean feed, nothing in their terms against reuse, our cards would publish a summary and a link, and it is still out, because they said they don't want AI taking their journalism. The way to reopen one is to ASK the publisher, not to re-measure the feed.
...
- **Always invoke the ingest scripts through `npm run` (2026-08-10).** Node's global `fetch` ignores `HTTPS_PROXY` unless the process is *started* with `NODE_USE_ENV_PROXY=1`, which the `ingest:fetch` / `ingest:geocode` scripts set. Run `node scripts/fetch-sources.mjs` bare in a proxied environment and every plain/feed/json source egresses direct — which the sandbox *intercepts*, so you get mangled bodies and phantom diffs, not honest failures. The script now refuses to start in that state; don't work around it by unsetting `HTTPS_PROXY`. See `docs/DECISION_LOG.md` 2026-08-10. **Since 2026-09-08 the cloud routine does not fetch roster sources at all:** the `ingest-fetch` GitHub Actions workflow reads the roster at 6:30am New York and a launchd job on Batu's Mac (`scripts/home-fetch.sh`, installed from `scripts/home-fetch.plist`; install steps in `docs/ops/home-fetch.md`) reads it again at 7:15 from home, because a GitHub runner is challenged by Cloudflare/Imperva on eight sources that a residential address reads fine. Both publish to `batusayici/stoopwise-snapshots`; the routine runs `ingest:pull` then `ingest:fetch -- --offline`. The sandbox allowlist therefore only needs github.com, Nominatim and one-off lookup hosts. If every source errors in a cloud run, look at the workflow run and the home log (`~/.stoopwise-fetch/home-fetch.log`) first, not the sandbox. The fetch script is identical in all three places; where it runs is a schedule, not code.
...
- **Never let git auto-merge `cards.json` across a content refresh (2026-08-30).** It is one big JSON array and the daily ingest rewrites large parts of it, so a branch cut even a few days back merges *cleanly* into a deck that is silently wrong — duplicated or resurrected cards, with no conflict markers to warn you. The pinned counts in `julyCards.test.mjs` are the only thing that catches it, and they catch it as a number mismatch that reads like a stale contract rather than a corrupted deck. **So: if a branch touching `cards.json` is behind `main`, do not merge or rebase it — rebuild it.** Take the code and docs across unchanged, then re-apply the data change against the current deck with a script and recompute the counts. This is why a branch that edits cards should be opened, reviewed and merged the same day.
...
## Working the repo (2026-09-14)

Audit of Jul 15–Sep 14: eight of forty PRs closed unmerged, every one a branch cut from `main` and left for days while a routine rewrote the same files underneath it; a false "main went backwards" alarm on 9/9 that came from a routine reading a stale copy of `main`; commits landing on another session's branch because sessions share one checkout. These rules exist so that stops.

- **Two lanes, split by file.** Routines own the content files (`cards.json`, `geocode-cache.json`, `ingest-ledger.json`, `ingest-sources.json`, `freshness-stamp.json`, `freshness-history.json`, `.ingest-cache/*.ingested.txt`, the contract counts in `julyCards.test.mjs`) and keep pushing them straight to `main`. Humans and interactive sessions own code and docs. **Do not hand-edit a content file on `main` between 9:45 and 11:00 New York** (the routine window); if a card must change by hand, do it after the routine has pushed, on a branch merged the same hour.
- **Every human change is a branch in its own worktree, a PR, and a same-day merge.** Never commit to `main` in the shared checkout; it stays parked on `main` for the routines' pull and for reading. Start a session's branch work with `git worktree add` (or the desktop app's worktree isolation), push the branch, open the PR, merge it, delete the branch. A branch older than two days is not merged or rebased; it is rebuilt against current `main` or closed with a note saying where its work went (the 2026-08-30 `cards.json` rule, now applied to every branch).
- **Routines start from GitHub's `main`, not the sandbox's copy.** First command of any routine: `git fetch origin && git reset --hard origin/main`. A rejected push means fetch, rebase, retry; it never means a PR. Before ever writing "main went backwards", confirm it with `gh api repos/batusayici/greenpoint-explorer/events` (push events to `refs/heads/main`); on 9/9 the log showed an unbroken forward chain and the alarm was the sandbox's stale clone.
- **`main` is protected against force-push and deletion** (GitHub ruleset `protect-main`, 2026-09-14). It does not require PRs, because the routines push to it directly. Rollback is `git revert`, never a reset.
- **Files every session regenerates are not committed.** `docs/launch/cockpit.html` left the repo on 2026-09-13 for this reason; analytics values live in `docs/growth/snapshots/`. Anything new that is rebuilt on every run goes the same way, or is written only by the routine lane.
- **Git defaults on the Mac:** `pull.rebase=true`, `push.autoSetupRemote=true`, `push.default=current` (set 2026-09-14). Archived branch tips live under `archive/*` tags; a branch that is merged or superseded gets deleted, not kept.
